...
harapan_shp = gpd.read_file('E:/SLICUAV_manuscript_data/7_Harapan_shapefiles/deforestation/'+'2020_04_17_Harapan_boundary_and_UAV_footprint_deforestation_masked_isolated_parts_removed_latlong.shp')
...
for shp_i in range(grid_shps.shape[0]):
    ...
    # Save it all
    pd.DataFrame(np.transpose(np.vstack((grid,seg,harapan)))).to_csv(':/SLICUAV_manuscript_data/5_Landscape_superpixels/Deforest_status/2020_04_17_harapan_non_deforest_status_' + str(ths_id) + '.csv',header=None,index=None)
