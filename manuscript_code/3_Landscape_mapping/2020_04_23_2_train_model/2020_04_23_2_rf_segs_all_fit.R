...
pptime <- toc()
write.csv(pptime$toc-pptime$tic,'2020_04_23_2_preproc_time_rf_all.csv')

# Save preprocessing machinery and also the variables which were used
save(preProcValues,file = '2020_04_23_2_preproc_time_rf_all.RData')
...
m_all <- randomForest::randomForest(
        x=X_trans,y=factor(Y[,1]),importance=TRUE,
        classwt = weights)
save(m_all,file='2020_04_23_2_all_classes_rf_model.RData')

preds<-predict(m_all,X_trans)
preds_table$rf_all <- as.character(preds)
rf_all_time <- toc()
write.csv(rf_all_time$toc-rf_all_time$tic,'2020_04_23_2_all_classes_rf_time.csv')

write.csv(preds_table,'2020_04_23_2_rf_predictions_all.csv')
