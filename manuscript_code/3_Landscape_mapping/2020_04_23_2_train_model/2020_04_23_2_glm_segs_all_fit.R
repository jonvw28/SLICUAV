...
pptime <- toc()
write.csv(pptime$toc-pptime$tic,'2020_04_23_2_preproc_time_GLM_all.csv')

# Save preprocessing machinery and also the variables which were used
save(preProcValues,file = '2020_04_23_2_preproc_time_GLM_all.RData')
...
weights <- numeric(N)
weights[Y[,1] == 'BEL'] <- N/(N_bel*7)
weights[Y[,1] == 'MAC'] <- N/(N_mac*7)
weights[Y[,1] == 'MIS'] <- N/(N_mis*7)
weights[Y[,1] == 'PAL'] <- N/(N_pal*7)
weights[Y[,1] == 'PUL'] <- N/(N_pul*7)
weights[Y[,1] == 'RAN'] <- N/(N_ran*7)
weights[Y[,1] == 'SEN'] <- N/(N_sen*7)
...
tic()
set.seed(42)
m_all <- glmnet::glmnet(X_trans, Y[,1], weights = weights, alpha = 1, family="multinomial", 
                          type.multinomial = "grouped")
save(m_all,file='2020_04_23_2_all_classes_glm_model.RData')

# Pick lambda that keeps at most 20 predictors
mdl_idx <- max(which(m_all$df<=N_max))
mdl_lam <- m_all$lambda[mdl_idx]

preds <- predict(m_all, newx = X_trans, s = mdl_lam, type = "class")
preds_table$glm_all <- as.character(preds)
glm_all_time <- toc()
write.csv(glm_all_time$toc-glm_all_time$tic,'2020_04_23_2_all_classes_glm_time.csv')

write.csv(preds_table,'2020_04_23_2_glm_predictions_all.csv')
