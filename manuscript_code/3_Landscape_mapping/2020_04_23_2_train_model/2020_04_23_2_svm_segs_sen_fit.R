...
pptime <- toc()
write.csv(pptime$toc-pptime$tic,'2020_04_23_2_preproc_time_SVM_sen.csv')

# Save preprocessing machinery and also the variables which were used
save(preProcValues,file = '2020_04_23_2_preproc_time_SVM_sen.RData')
...
m_sen <- e1071::best.svm(
                    x=X_trans,y=factor(preds_table$label_sen),
...
save(m_sen,file='2020_04_23_2_no_sen_classes_svm_model.RData')

preds<-predict(m_sen,X_trans)
preds_table$svm_sen <- as.character(preds)
svm_sen_time <- toc()
write.csv(svm_sen_time$toc-svm_sen_time$tic,'2020_04_23_2_no_sen_classes_svm_time.csv')

write.csv(preds_table,'2020_04_23_2_svm_predictions_sen.csv')
