# Fit various forms of RF models to the crown segment data using uncorrected MS data
...
setwd("E:/SLICUAV_manusript_code/1_assessing_models/2020_04_21_2_superpixel_cv_tests/RF")
...
pptime <- toc()
write.csv(pptime$toc-pptime$tic,paste('2020_04_21_2_preproc_time_RF__fold_',fold,'.csv',sep=''))

# Save preprocessing machinery and also the variables which were used
save(preProcValues,file = paste('2020_04_21_2_preproc_RF_fold_',fold,'.RData',sep=''))
...
m_all <- randomForest::randomForest(
        x=train_X_trans,y=factor(train_Y),importance=TRUE,
        classwt = weights)
save(m_all,file = paste('2020_04_21_2_RF_fold_',fold,'_all_classes_RF_cluster_model.RData',sep=''))
...
RF_all_time <- toc()
write.csv(RF_all_time$toc-RF_all_time$tic,paste('2020_04_21_2_all_classes_RF_time_fold_',fold,'.csv',sep=''))

# tidy
rm(m_all,train_pred,test_pred,weights)

write.csv(preds_table,paste('2020_04_21_2_fold_',fold,'_RF_predictions_pre_sen.csv',sep=''))
...
###################### Fit RF model on no sen data ############################
tic()
set.seed(42)
m_sen <- randomForest::randomForest(
        x=train_X_trans,y=factor(train_Y_mods),importance=TRUE,
        classwt = weights)
save(m_sen,file = paste('2020_04_21_2_RF_fold_1_',fold,'_drop_sen_RF_cluster_model.RData',sep=''))
...
RF_sen_time <- toc()
write.csv(RF_sen_time$toc-RF_sen_time$tic,paste('2020_04_21_2_drop_sen_RF_time_fold_',fold,'.csv',sep=''))

# tidy
rm(m_sen,train_pred,test_pred,weights)

write.csv(preds_table,paste('2020_04_21_2_fold_',fold,'_RF_predictions_pre_ran.csv',sep=''))
...
###################### Fit RF model on no sen or ran data #####################
tic()
set.seed(42)
m_ran <- randomForest::randomForest(
        x=train_X_trans,y=factor(train_Y_modsp),importance=TRUE,
        classwt = weights)
save(m_ran,file = paste('2020_04_21_2_RF_fold_',fold,'_drop_sen_pul_RF_cluster_model.RData',sep=''))
...
RF_ran_time <- toc()
write.csv(RF_ran_time$toc-RF_ran_time$tic,paste('2020_04_21_2_drop_pul_sen_RF_time_fold_',fold,'.csv',sep=''))
...
write.csv(preds_table,paste('2020_04_21_2_fold_',fold,'_RF_predictions.csv',sep=''))
