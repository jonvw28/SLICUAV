...
preds_table <- data.frame(ttagstring = Y[good_MS_mask,2],cluster = Y[good_MS_mask,3],label = Y[good_MS_mask,1],
                          ...
                          label_ran = temp_vec,
                          split = temp_vec,
                          text_feats_DSM_imagery_all = temp_vec,
                          text_feats_DSM_imagery_mods = temp_vec,
                          text_feats_DSM_imagery_sen = temp_vec,
                          text_feats_DSM_imagery_modsp = temp_vec,
                          text_feats_DSM_imagery_ran = temp_vec,
                          stringsAsFactors = F
                          )
...
# 
var_mask <-intersect(text_vars,dsm_vars)
mod_string <- 'text_feats_DSM_imagery'

tic()
train_X <- X[train_idx,var_mask]
train_Y <- Y[train_idx,1]
test_X <- X[!train_idx,var_mask]
